...
class CRF(nn.Module):
    ...
    def __init__(
        self, nb_labels, bos_tag_id, eos_tag_id, cuda,pad_tag_id=None, batch_first=True
    ):
        super().__init__()
        ...
        self.transitions = nn.Parameter(torch.empty(self.nb_labels, self.nb_labels)).to(cuda)
        self.init_weights()
    ...
    def init_weights(self):
        ...
        # self.transitions.data[:, self.BOS_TAG_ID] = -10.0
        # self.transitions.data[self.BOS_TAG_ID, :] = 10.0
        # # no transition alloed from the end of sentence
        # self.transitions.data[self.EOS_TAG_ID, :] = -10.0

        # if self.PAD_TAG_ID is not None:
        #     # no transitions from padding
            # self.transitions.data[self.PAD_TAG_ID, :] = -10.0
    ...
    def log_likelihood(self, emissions, tags, mask=None):
        ...
        if mask is None:
            mask = torch.ones(emissions.shape[:2], dtype=torch.float).to(self.cuda)
    ...
    def _compute_scores(self, emissions, tags, mask):
        ...
        scores = torch.zeros(batch_size).to(self.cuda)
